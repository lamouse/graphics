...
#include "shader_tools/exception.hpp"
namespace shader::IR {
auto isGeneric(Attribute attribute) noexcept -> bool {
    return attribute >= Attribute::Generic0X && attribute <= Attribute::Generic31X;
}

auto genericAttributeIndex(Attribute attribute) -> uint32_t {
    if (!isGeneric(attribute)) {
        throw InvalidArgument("Attribute is not generic {}", attribute);
    }
    return (static_cast<uint32_t>(attribute) - static_cast<uint32_t>(Attribute::Generic0X)) / 4u;
}

auto genericAttributeElement(Attribute attribute) -> uint32_t {
    if (!isGeneric(attribute)) {
        throw InvalidArgument("Attribute is not generic {}", attribute);
    }
    return static_cast<uint32_t>(attribute) % 4;
}

auto nameOf(Attribute attribute) -> std::string {
    switch (attribute) {
