...
class RasterizerInterface {
    public:
        ...
        /// memory
        virtual void FlushRegion(DAddr addr, u64 size, CacheType which = CacheType::All) = 0;

        /// Check if the the specified memory area requires flushing to CPU Memory.
        virtual bool MustFlushRegion(DAddr addr, u64 size, CacheType which = CacheType::All) = 0;

        virtual RasterizerDownloadArea GetFlushArea(DAddr addr, u64 size) = 0;

        /// Notify rasterizer that any caches of the specified region should be invalidated
        virtual void InvalidateRegion(DAddr addr, u64 size, CacheType which = CacheType::All) = 0;
        ...
        virtual void FlushAndInvalidateRegion(DAddr addr, u64 size,
                                              CacheType which = CacheType::All) = 0;

        /// Notify the host renderer to wait for previous primitive and compute operations.
        virtual void WaitForIdle() = 0;

        /// Notify the host renderer to wait for reads and writes to render targets and flush
        /// caches.
        ...
        virtual auto AccelerateConditionalRendering() -> bool { return false; }
