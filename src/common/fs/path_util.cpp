...
class PathManagerImpl {
    public:
        ...
        [[nodiscard]] const fs::path& GetSuyuPathImpl(EnginePath suyu_path) {
            return suyu_paths.at(suyu_path);
        }

        void SetSuyuPathImpl(EnginePath suyu_path, const fs::path& new_path) {
        ...
        void Reinitialize(fs::path suyu_path = {}) {
...
#endif
            if (!IsDir(suyu_path)) {
                suyu_path = GetAppDataRoamingDirectory() / "graphics";
            }
            ...
            GenerateSuyuPath(EnginePath::DataDir, suyu_path);

            GenerateSuyuPath(EnginePath::CacheDir, suyu_path_cache);
            GenerateSuyuPath(EnginePath::ConfigDir, suyu_path_config);
            GenerateSuyuPath(EnginePath::CrashDumpsDir, suyu_path / CRASH_DUMPS_DIR);
            GenerateSuyuPath(EnginePath::DumpDir, suyu_path / DUMP_DIR);
            GenerateSuyuPath(EnginePath::IconsDir, suyu_path / ICONS_DIR);
            GenerateSuyuPath(EnginePath::KeysDir, suyu_path / KEYS_DIR);
            GenerateSuyuPath(EnginePath::LoadDir, suyu_path / LOAD_DIR);
            GenerateSuyuPath(EnginePath::NANDDir, suyu_path / NAND_DIR);
            GenerateSuyuPath(EnginePath::PlayTimeDir, suyu_path / PLAY_TIME_DIR);
            GenerateSuyuPath(EnginePath::ShaderDir, suyu_path / SHADER_DIR);
            GenerateSuyuPath(EnginePath::TASDir, suyu_path / TAS_DIR);
        }

    private:
        PathManagerImpl() { Reinitialize(); }

        ~PathManagerImpl() = default;

        void GenerateSuyuPath(EnginePath suyu_path, const fs::path& new_path) {
            void(FS::CreateDir(new_path));

            SetSuyuPathImpl(suyu_path, new_path);
        }

        std::unordered_map<EnginePath, fs::path> suyu_paths;
};

bool ValidatePath(const fs::path& path) {
    if (path.empty()) {
        SPDLOG_ERROR("Input path is empty, path={}", PathToUTF8String(path));
        return false;
    }

#ifdef _WIN32
    if (path.u16string().size() >= MAX_PATH) {
        SPDLOG_ERROR("Input path is too long, path={}", PathToUTF8String(path));
        return false;
    }
#else
...
bool IsDirSeparator(char character) { return character == '/' || character == '\\'; }

bool IsDirSeparator(char8_t character) { return character == u8'/' || character == u8'\\'; }
...
const fs::path& GetSuyuPath(EnginePath suyu_path) {
    return PathManagerImpl::GetInstance().GetSuyuPathImpl(suyu_path);
}

std::string GetSuyuPathString(EnginePath suyu_path) {
    return PathToUTF8String(GetSuyuPath(suyu_path));
}

void SetSuyuPath(EnginePath suyu_path, const fs::path& new_path) {
    if (!FS::IsDir(new_path)) {
        SPDLOG_ERROR("Filesystem object at new_path={} is not a directory",
...
fs::path GetExeDirectory() {
    ...
    if (GetModuleFileNameW(nullptr, exe_path, MAX_PATH) == 0) {
        SPDLOG_ERROR("Failed to get the path to the executable of the current process");
    }
...
fs::path GetAppDataRoamingDirectory() {
    ...
    if (fs_appdata_roaming_path.empty()) {
        SPDLOG_ERROR("Failed to get the path to the %APPDATA% directory");
    }
...
fs::path GetHomeDirectory() {
    ...
    if (!pw) {
        SPDLOG_ERROR("Failed to get the home path of the current user");
        return {};
    }
