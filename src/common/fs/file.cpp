...
fs::path IOFile::GetPath() const { return file_path; }

FileAccessMode IOFile::GetAccessMode() const { return file_access_mode; }

FileType IOFile::GetType() const { return file_type; }
...
void IOFile::Open(const fs::path& path, FileAccessMode mode, FileType type, FileShareFlag flag) {
    ...
    if (Android::IsContentUri(path)) {
        ...
        if (fd != -1) {
            ...
            if (error_num != 0 && file == nullptr) {
                SPDLOG_ERROR("Error opening file: {}, error: {}", path.c_str(),
                             strerror(error_num));
            }
        } else {
            SPDLOG_ERROR("Error opening file: {}", path.c_str());
        }
    } else {
    ...
    if (!IsOpen()) {
        ...
        SPDLOG_ERROR("Failed to open the file at path={}, ec_message={}",
...
void IOFile::Close() {
    ...
    if (!close_result) {
        const auto ec = std::error_code{errno, std::generic_category()};
        SPDLOG_ERROR("Failed to close the file at path={}, ec_message={}",
                     PathToUTF8String(file_path), ec.message());
    }
...
size_t IOFile::WriteString(std::span<const char> string) const { return WriteSpan(string); }
...
bool IOFile::Flush() const {
    ...
    if (!flush_result) {
        const auto ec = std::error_code{errno, std::generic_category()};
        SPDLOG_ERROR("Failed to flush the file at path={}, ec_message={}",
                     PathToUTF8String(file_path), ec.message());
    }
...
bool IOFile::Commit() const {
    ...
    if (!commit_result) {
        const auto ec = std::error_code{errno, std::generic_category()};
        SPDLOG_ERROR("Failed to commit the file at path={}, ec_message={}",
                     PathToUTF8String(file_path), ec.message());
    }
...
bool IOFile::SetSize(u64 size) const {
    ...
    if (!set_size_result) {
        const auto ec = std::error_code{errno, std::generic_category()};
        SPDLOG_ERROR("Failed to resize the file at path={}, size={}, ec_message={}",
...
u64 IOFile::GetSize() const {
    ...
    if (Android::IsContentUri(file_path)) {
        ...
        if (ec) {
            SPDLOG_ERROR(Common_Filesystem,
...
bool IOFile::Seek(s64 offset, SeekOrigin origin) const {
    ...
    if (!seek_result) {
        const auto ec = std::error_code{errno, std::generic_category()};
        SPDLOG_ERROR("Failed to seek the file at path={},  ec_message={}",
                     PathToUTF8String(file_path), ec.message());
    }
