...
[[nodiscard]] auto ValidatePath(const Path& path) -> bool {
...
[[nodiscard]] auto ConcatPath(const Path1& first, const Path2& second) -> std::filesystem::path {
 ...
 */
[[nodiscard]] auto IsPathSandboxed(const std::filesystem::path& base,
                                   const std::filesystem::path& path) -> bool;

#ifdef _WIN32
template <typename Path1, typename Path2>
[[nodiscard]] auto IsPathSandboxed(const Path1& base, const Path2& path) -> bool {
    using ValueType1 = typename Path1::value_type;
    using ValueType2 = typename Path2::value_type;
    if constexpr (IsChar<ValueType1> && IsChar<ValueType2>) {
 ...
 */
[[nodiscard]] const std::filesystem::path& GetSuyuPath(EnginePath suyu_path);
 ...
 */
[[nodiscard]] std::string GetSuyuPathString(EnginePath suyu_path);
 ...
 */
void SetSuyuPath(EnginePath suyu_path, const std::filesystem::path& new_path);

#ifdef _WIN32
template <typename Path>
void SetSuyuPath(EnginePath suyu_path, const Path& new_path) {
    if constexpr (IsChar<typename Path::value_type>) {
