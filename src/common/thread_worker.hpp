...
class StatefulThreadWorker {
        ...
        using StateMaker =
            std::conditional_t<with_state, std::function<StateType()>, DummyCallable>;
    ...
    public:
        explicit StatefulThreadWorker(size_t num_workers, std::string name, StateMaker func = {})
            ...
            const auto lambda = [this, func](std::stop_token stop_token) {
                common::thread::SetCurrentThreadName(thread_name.c_str());
                {
                    ...
                    while (!stop_token.stop_requested()) {
                        Task task;
                        {
                            ...
                            common::thread::condvarWait(condition, lock, stop_token,
