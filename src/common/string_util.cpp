...
std::string StringFromBool(bool value) { return value ? "True" : "False"; }
...
// windows needs the : included for something like just "C:" to be considered a directory
    ...
    if (fname_end < dir_end || std::string::npos == fname_end) fname_end = full_path.size();

    if (_pPath) *_pPath = full_path.substr(0, dir_end);

    if (_pFilename) *_pFilename = full_path.substr(dir_end, fname_end - dir_end);

    if (_pExtension) *_pExtension = full_path.substr(fname_end);
...
std::wstring UTF8ToUTF16W(std::string_view input) { return CPToUTF16(CP_UTF8, input); }
