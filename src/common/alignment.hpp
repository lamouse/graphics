 ...
 * @return T The aligned value, which is the smallest multiple of size that is greater than or equal to value_.
 */
template <typename T>
...
[[nodiscard]] constexpr T alignUp(T value_, size_t size) {
    ...
    return static_cast<T>(mod == T{0} ? value : value + size);  // If mod is not zero, add size to value
}
