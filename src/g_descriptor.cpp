...
auto DescriptorSetLayout::Builder::addBinding(uint32_t binding,
  ...
  assert(not descriptorSetLayoutBindings_.contains(binding) &&
...
auto DescriptorSetLayout::Builder::build()
    -> ::std::unique_ptr<DescriptorSetLayout> {
  return ::std::make_unique<DescriptorSetLayout>(device_,
...
DescriptorSetLayout::DescriptorSetLayout(
  ...
  descriptorSetLayout_ =
      device_.logicalDevice().createDescriptorSetLayout(setLayoutCreateInfo);
}
...
auto DescriptorPool::Builder::addPoolSize(::vk::DescriptorType type,
                                          uint32_t count)
...
auto DescriptorPool::Builder::setPoolFlags(
    ::vk::DescriptorPoolCreateFlags flags) -> DescriptorPool::Builder & {
  flags_ = flags;
  return *this;
}

auto DescriptorPool::Builder::setMaxSets(uint32_t maxSets)
    -> DescriptorPool::Builder & {
  maxSets_ = maxSets;
  return *this;
}
...
auto DescriptorPool::Builder::build() -> ::std::unique_ptr<DescriptorPool> {
  return ::std::make_unique<DescriptorPool>(device_, maxSets_, flags_,
                                            poolSizes_);
}

DescriptorPool::DescriptorPool(
    ::core::Device &device, uint32_t maxSets,
...
void DescriptorPool::allocateDescriptor(
  ...
  descriptorSet =
      device_.logicalDevice().allocateDescriptorSets(allocateInfo)[0];
}
...
DescriptorWriter::DescriptorWriter(DescriptorSetLayout &descriptorSetLayout,
                                   DescriptorPool &pool)
    : descriptorSetLayout_(descriptorSetLayout), pool_(pool) {}

auto DescriptorWriter::writeBuffer(uint32_t binding,
                                   ::vk::DescriptorBufferInfo &bufferInfo)
    -> DescriptorWriter & {
  auto &bindingDescrptor =
      descriptorSetLayout_.descriptorSetLayoutBindings_[binding];
  assert(bindingDescrptor.descriptorCount == 1 &&
...
auto DescriptorWriter::writeImage(uint32_t binding,
...
void DescriptorWriter::build(::vk::DescriptorSet &descriptorSet) {
  pool_.allocateDescriptor(descriptorSetLayout_.getDescriptorSetLayout(),
                           descriptorSet);
...
void DescriptorWriter::overwrite(::vk::DescriptorSet &descriptorSet) {
  ...
  pool_.device_.logicalDevice().updateDescriptorSets(
